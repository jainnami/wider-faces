...
setwd("/Users/namijain/WiderFaces/wider-faces")
...
process_image <- function(cat, img_file) {
  img_path <- paste0("WIDER_test/images/", cat, "/", img_file)
...
for (cat in categories) {
  img_files <- list.files(paste0("WIDER_test/images/", cat))
  # Sample 10 images, or all if less
  n_sample <- min(20, length(img_files))
